...
var fetchDateRange = function(username, startDate, endDate, fields, callback) {
  ...
  request(options, function(error, response, body) {
    ...
    $(".main-title-2").each(function(index, element) {
      ...
      var exerciseTable =
        $element.next("#food").length > 0
          ? $element.next().next()
          : $element.next("#excercise");
    ...
    var processDate = function(date, $table, $excerciseTable) {
      ...
      $table
        .find("td:contains('Green Tea - Green Tea')")
        .each(function(index, element) {
          var cupsOfTea = Number(
            $(element)
              .text()
              .charAt(23)
          );
      ...
      $table
        ...
        .each(function(index, element) {
          ...
          if (index !== 0) {
            cols.nutrition[fieldName] = index;
      ...
      $excerciseTable
        ...
        .each(function(index, element) {
          ...
          if (
            $element.find("td:first-of-type").text() !==
            "MFP iOS calorie adjustment"
          ) {
            var excerciseData = {
              name: $element.find("td:first-of-type").text(),
              calories: helpers.convertToNum(
                $element.find("td:nth-of-type(2)").text()
              ),
              minutes: helpers.convertToNum(
                $element.find("td:nth-of-type(3)").text()
              )
            };
